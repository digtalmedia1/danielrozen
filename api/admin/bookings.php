...
if ($method === "POST") {
    ...
    if ($action === "manual_create") {
        ...
        try {
            // Overlap check
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM availability_slots WHERE ((start_datetime < ? AND end_datetime > ?) OR (start_datetime < ? AND end_datetime > ?)) AND status != 'available'");
            ...
            logAudit($pdo, 'manual_booking_created', ['booking_id' => $bookingId, 'slot_id' => $slotId]);
            $pdo->commit();
